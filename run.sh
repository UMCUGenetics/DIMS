...
find $outdir/data -iname "*.mzML" | sort | while read mzML;
 do
     ...
     if [ \$it -lt 3 ] && [ ! -s $outdir/breaks.fwhm.RData ] ; then
       ...
       qsub -q all.q -P dbg_mz -l h_rt=00:05:00 -l h_vmem=1G -N "breaks_${name}" -m as -M $email -o $outdir/logs/1-generateBreaksFwhm.HPC -e $outdir/logs/1-generateBreaksFwhm.HPC $outdir/jobs/1-generateBreaksFwhm.HPC/breaks.sh
     fi
     ...
     qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=4G -N "dims_\${input}_${name}" -hold_jid "breaks_${name}" -m as -M $email -o $outdir/logs/2-DIMS -e $outdir/logs/2-DIMS $outdir/jobs/2-DIMS/\${input}.sh
...
qsub -q all.q -P dbg_mz -l h_rt=01:30:00 -l h_vmem=5G -N "average_${name}" -hold_jid "dims_*_${name}" -m as -M $email -o $outdir/logs/3-averageTechReplicates -e $outdir/logs/3-averageTechReplicates $outdir/jobs/3-averageTechReplicates/average.sh

qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=500M -N "queueFinding_positive_${name}" -hold_jid "average_${name}" -m as -M $email -o $outdir/logs/queue/2-queuePeakFinding -e $outdir/logs/queue/2-queuePeakFinding $outdir/jobs/queue/2-queuePeakFinding_positive.sh
qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=500M -N "queueFinding_negative_${name}" -hold_jid "average_${name}" -m as -M $email -o $outdir/logs/queue/2-queuePeakFinding -e $outdir/logs/queue/2-queuePeakFinding $outdir/jobs/queue/2-queuePeakFinding_negative.sh
EOF
...
if [ "\$1" -lt 1 ]; then # first check
  ...
  do
    ...
    if [ ! -s $outdir/data/\${file}.mzML ]; then
      ...
      continue=false
      qsub -q all.q -P dbg_mz -l h_rt=00:03:00 -l h_vmem=4G -N "conversion_\${file}_${name}" -m asb -M $email -o $outdir/logs/0-conversion -e $outdir/logs/0-conversion $outdir/jobs/0-conversion/\${file}.sh
  	fi
  done
...
if [ "\$continue" = true ]; then
  echo continue
  qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=1G -N "queueStart_${name}" -hold_jid "conversion_*_${name}" -m as -M $email -o $outdir/logs/queue/1-queueStart -e $outdir/logs/queue/1-queueStart $outdir/jobs/queue/1-queueStart.sh
else
  echo redo conversion check
  qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=1G -N "queueConversionCheck_${name}" -hold_jid "conversion_*_${name}" -m as -M $email -o $outdir/logs/queue/ -e $outdir/logs/queue/ $outdir/jobs/queue/01-queueConversionCheck.sh 1
fi
...
find $indir -iname "*.raw" | sort | while read raw;
  do
    ...
    qsub -q all.q -P dbg_mz -l h_rt=00:03:00 -l h_vmem=4G -N "conversion_${input}_${name}" -m s -M $email -o $outdir/logs/0-conversion -e $outdir/logs/0-conversion $outdir/jobs/0-conversion/${input}.sh
  done

qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=1G -N "queueConversionCheck_${name}" -hold_jid "conversion_*_${name}" -m as -M $email -o $outdir/logs/queue/01-queueConversionCheck -e $outdir/logs/queue/01-queueConversionCheck $outdir/jobs/queue/01-queueConversionCheck.sh 0
...
if [ -f "$outdir/logs/done" ]; then   # if one of the scanmodes is already queued
  ...
  qsub -q all.q -P dbg_mz -l h_rt=01:00:00 -l h_vmem=8G -N "excelExport_${name}" -hold_jid "collect3_*_${name}" -m ase -M $email -o $outdir/logs/13-excelExport -e $outdir/logs/13-excelExport $outdir/jobs/13-excelExport.sh
  qsub -q all.q -P dbg_mz -l h_rt=00:10:00 -l h_vmem=1G -N "cleanup_${name}" -hold_jid "excelExport_${name}" -m as -M $email -o $outdir/logs/14-cleanup -e $outdir/logs/14-cleanup $outdir/jobs/14-cleanup.sh
else
