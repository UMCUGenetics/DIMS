...
for mzML in ${outdir}/1-data/*.mzML ; do
  ...
  if [ ! -v break_id ] ; then
    ...
    singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/1-generateBreaksFwhm.HPC.R \$mzML ${outdir} ${trim} ${resol} ${nrepl} ${scripts}
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/2-DIMS.R \$mzML ${outdir} ${trim} ${dims_thresh} ${resol} ${scripts}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/3-averageTechReplicates.R ${indir} ${outdir} ${nrepl} ${thresh2remove} ${dims_thresh} ${scripts}
