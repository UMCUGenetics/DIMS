...
sbatch --job-name=1-queueStart_${name} --time=${queue_1_time} --mem=${queue_1_mem} --dependency=afterok:\${job_ids}:+10 --output=${outdir}/logs/queue/1-queueStart.o --error=${outdir}/logs/queue/1-queueStart.e ${global_sbatch_parameters} ${outdir}/jobs/queue/1-queueStart.sh
EOF
...
col_id=\$(sbatch --job-name=5-collectSamples_${scanmode}_${name} --time=${job_5_time} --mem=${job_5_mem} --dependency=afterok:\${job_ids}:+10 --output=${outdir}/logs/5-collectSamples/${scanmode}.o --error=${outdir}/logs/5-collectSamples/${scanmode}.e ${global_sbatch_parameters} ${outdir}/jobs/5-collectSamples/${scanmode}.sh)
...
# start next queue
sbatch --job-name=5-queueFillMissing_${scanmode}_${name} --time=${queue_5_time} --mem=${queue_5_mem} --dependency=afterok:\${job_ids}:+10 --output=${outdir}/logs/queue/5-queueFillMissing_${scanmode}.o --error=${outdir}/logs/queue/5-queueFillMissing_${scanmode}.e ${global_sbatch_parameters} ${outdir}/jobs/queue/5-queueFillMissing_${scanmode}.sh
EOF
...
col_id=\$(sbatch --job-name=10-collectSamplesFilled_${scanmode}_${name} --time=${job_10_time} --mem=${job_10_mem} --dependency=afterok:\${job_ids}:+10 --output=${outdir}/logs/10-collectSamplesFilled/${scanmode}.o --error=${outdir}/logs/10-collectSamplesFilled/${scanmode}.e ${global_sbatch_parameters} ${outdir}/jobs/10-collectSamplesFilled/${scanmode}.sh)

# start next queue
sbatch --job-name=6-queueSumAdducts_${scanmode}_${name} --time=${queue_6_time} --mem=${queue_6_mem} --dependency=afterok:\${col_id}:+10 --output=${outdir}/logs/queue/6-queueSumAdducts_${scanmode}.o --error=${outdir}/logs/queue/6-queueSumAdducts_${scanmode}.e ${global_sbatch_parameters} ${outdir}/jobs/queue/6-queueSumAdducts_${scanmode}.sh
EOF
...
col_id=\$(sbatch --job-name=12-collectSamplesAdded_${scanmode}_${name} --time=${job_12_time} --mem=${job_12_mem} --dependency=afterok:\${job_ids}:+10 --output=${outdir}/logs/12-collectSamplesAdded/${scanmode}.o --error=${outdir}/logs/12-collectSamplesAdded/${scanmode}.e ${global_sbatch_parameters} ${outdir}/jobs/12-collectSamplesAdded/${scanmode}.sh)
...
if [ -f "${outdir}/logs/done" ]; then   # if one of the scanmodes has already finished
  ...
  exp_id=\$(sbatch --job-name=13-excelExport_${name} --time=${job_13_time} --mem=${job_13_mem} --dependency=afterok:\${col_ids}:+10 --output=${outdir}/logs/13-excelExport/exp.o --error=${outdir}/logs/13-excelExport/exp.e ${global_sbatch_parameters} ${outdir}/jobs/13-excelExport.sh)
  sbatch --job-name=14-cleanup_${name} --time=${job_14_time} --mem=${job_14_mem} --dependency=afterok:\${exp_id}:+10 --output=${outdir}/logs/14-cleanup.o --error=${outdir}/logs/14-cleanup.e ${global_sbatch_parameters} ${outdir}/jobs/14-cleanup.sh
else
