...
for mzML in ${outdir}/1-data/*.mzML ; do
  ...
  if [ ! -v break_id ] ; then
    ...
    singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/1-generateBreaksFwhm.HPC.R \$mzML ${outdir} ${trim} ${resol} ${nrepl} ${scripts} ${stitch}
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/2-DIMS.R \$mzML ${outdir} ${trim} ${dims_thresh} ${resol} ${scripts} ${stitch}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/3-averageTechReplicates.R ${indir} ${outdir} ${nrepl} ${thresh2remove} ${dims_thresh} ${scripts} ${stitch}
...
for sample in ${outdir}/3-average_pklist/*${label}* ; do
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/4-peakFinding.R \${sample} ${outdir} ${scanmode} ${thresh} ${resol} ${scripts}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/5-collectSamples.R ${outdir} ${scanmode}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/hmdb_part_adductSums.R ${outdir} ${scanmode} ${db}
...
for hmdb in ${outdir}/hmdb_part/${scanmode}_* ; do
  ...
  # 6-peakGrouping
  echo "#!/bin/sh
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/6-peakGrouping.R \$hmdb ${outdir} ${scanmode} ${resol} ${ppm}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/7-collectSamplesGroupedHMDB.R ${outdir} ${scanmode} ${ppm}
...
for file in ${outdir}/7-specpks_all_rest/${scanmode}_* ; do
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/8-peakGrouping.rest.R \$file ${outdir} ${scanmode} ${resol} ${ppm}
...
for file in ${outdir}/8-grouping_rest/${scanmode}_* ; do
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/9-runFillMissing.R \$file ${outdir} ${scanmode} ${thresh} ${resol} ${scripts} ${ppm}
...
for file in ${outdir}/6-grouping_hmdb/*_${scanmode}.RData ; do
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/9-runFillMissing.R \$file ${outdir} ${scanmode} ${thresh} ${resol} ${scripts}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/10-collectSamplesFilled.R ${outdir} ${scanmode} ${normalization} ${scripts} ${z_score}  ${ppm}
...
for hmdb in ${outdir}/hmdb_part_adductSums/${scanmode}_* ; do
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/11-runSumAdducts.R \$hmdb ${outdir} ${scanmode} ${adducts} ${z_score}
...
singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/12-collectSamplesAdded.R ${outdir} ${scanmode} ${scripts}
...
if [ -f "${outdir}/logs/done" ]; then   # if one of the scanmodes has already finished
  ...
  singularity -q exec -B /hpc/dbg_mz:/hpc/dbg_mz -B $TMPDIR:$TMPDIR ${dims_r_pack_image_path} Rscript ${scripts}/13-excelExport.R ${outdir} ${name} ${matrix} ${db2} ${z_score}
