...
TrimlowerMZrange <- function(hdr) {
  ...
  for (i in 1:length(hdr$scanWindowLowerLimit)) {
    if (hdr$scanWindowLowerLimit[[i]] == min(hdr$scanWindowLowerLimit)) {cat(',', hdr$scanWindowLowerLimit[[i]])}
...
TrimupperMZrange <- function(hdr) {
  ...
  for (i in 1:length(hdr$scanWindowUpperLimit)) {
    if (hdr$scanWindowUpperLimit[[i]] == max(hdr$scanWindowUpperLimit)) {cat(',', hdr$scanWindowUpperLimit[[i]])}  
...
#x <- suppressMessages(xcmsRaw(filepath)) # previous code
Dat <- openMSfile(filepath)
hdr <- header(Dat)
pks <- spectra(Dat)
...
# Generate a matrix
#y <- rawMat(x) # previous code
# unlike xcms, mzR doesn't have a matrix in the format of y
# making a proxy in this for loop:
