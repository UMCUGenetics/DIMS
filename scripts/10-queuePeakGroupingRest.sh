...
qsub -l h_rt=01:00:00 -l h_vmem=8G -N "queueFillMissing_$scanmode" -hold_jid "grouping2_$scanmode" -m as -M $MAIL -o $LOGDIR/'$JOB_NAME.txt' -e $LOGDIR/'$JOB_NAME.txt' $SCRIPTS/12-queuefillMissing.sh $INDIR $OUTDIR $SCRIPTS $LOGDIR $ERRORS $MAIL $scanmode $thresh $label $adducts
