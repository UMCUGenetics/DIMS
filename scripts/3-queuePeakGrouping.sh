...
find "$OUTDIR/hmdb_part" -iname "${scanmode}_*" | sort | while read hmdb;
 do
   it=$((it+1))
   echo "Rscript $SCRIPTS/R/6-peakGrouping.2.0.R $hmdb $OUTDIR $scanmode $resol $SCRIPTS/R" > $OUTDIR/jobs/6-peakGrouping.2.0_${scanmode}_${it}.sh
   qsub -l h_rt=01:00:00 -l h_vmem=8G -N "grouping_${scanmode}_${it}" -m as -M $MAIL -o $OUTDIR/logs/6-peakGrouping.2.0 -e $OUTDIR/logs/6-peakGrouping.2.0 $OUTDIR/jobs/6-peakGrouping.2.0_${scanmode}_${it}.sh
 done
