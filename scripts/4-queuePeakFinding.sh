...
find "$OUTDIR/average_pklist" -iname $label | while read sample;
 do
     qsub -l h_rt=00:30:00 -l h_vmem=8G -N "peakFinding_$scanmode" -m as -M $MAIL -o $JOBS -e $ERRORS $SCRIPTS/5-runPeakFinding.sh $sample $OUTDIR $scanmode $thresh $resol $SCRIPTS/R
     #Rscript peakFinding.2.0.R $sample $SCRIPTS $OUTDIR $thresh $resol $scanmode
 done

qsub -l h_rt=00:15:00 -l h_vmem=8G -N "collect_$scanmode" -hold_jid "peakFinding_$scanmode" -m as -M $MAIL -o $JOBS -e $ERRORS $SCRIPTS/6-runCollectSamples.sh $OUTDIR $scanmode $SCRIPTS/R
