...
find "$OUTDIR/grouping_hmdb" -iname "*_${scanmode}.RData" | while read rdata2;
 do
  qsub -l h_rt=02:00:00 -l h_vmem=8G -N "peakFilling2_$scanmode" -hold_jid "peakFilling_$scanmode" -m as -M $MAIL -o $LOGDIR/'$JOB_NAME'"_${rdata2}.txt" -e $LOGDIR/'$JOB_NAME'"_${rdata2}.txt" $SCRIPTS/13-runFillMissing.sh $rdata2 $OUTDIR $scanmode $thresh $resol $SCRIPTS/R
