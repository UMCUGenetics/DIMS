...
find "$OUTDIR/hmdb_part" -iname "${scanmode}_*" | while read hmdb;
 do
     qsub -l h_rt=01:00:00 -l h_vmem=8G -N "grouping_$scanmode" -m as -M $MAIL -o $JOBS -e $ERRORS $SCRIPTS/8-runPeakGrouping.sh $hmdb $OUTDIR $SCRIPTS/R $scanmode $resol
     #Rscript peakGrouping.2.0.R $hmdb $SCRIPTS $OUTDIR $resol $scanmode
 done

qsub -l h_rt=00:15:00 -l h_vmem=8G -N "collect1_$scanmode" -hold_jid "grouping_$scanmode" -m as -M $MAIL -o $JOBS -e $ERRORS $SCRIPTS/9-runCollectSamplesGroupedHMDB.sh $OUTDIR $scanmode
