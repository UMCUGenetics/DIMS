...
find "$OUTDIR/average_pklist" -iname $label | sort | while read sample;
 do
   ...
   qsub -l h_rt=00:30:00 -l h_vmem=8G -N "peakFinding_${scanmode}_${it}" -m as -M $MAIL -o $OUTDIR/logs/4-peakFinding.2.0 -e $OUTDIR/logs/4-peakFinding.2.0 $OUTDIR/jobs/peakFinding_${scanmode}_${it}.sh
 done
