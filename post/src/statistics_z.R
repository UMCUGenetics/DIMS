  ...
  if (plot) {  
    ...
    for (p in 1:nrow(peaklist)) { # p <- 1119 # in Neg  # p <- 2658  # in Pos
      ...
      for (i in 1:length(patients)){
        label=colnames(peaklist)[int.cols][grep(paste(case_label, patients[i], ".", sep=""), colnames(peaklist)[int.cols], fixed = TRUE)]
        # p.int=data.matrix(peaklist[p,label])
        p.int=as.numeric(as.vector(unlist(peaklist[p,label])))
        
        # assign(paste("P",i,sep=""),p.int[1,])
        assign(paste("P",i,sep=""),p.int)
        vl[[i+1]]=get(paste("P",i,sep=""))
        labels=c(labels,toString(patients[i]))
      }
      
      vl = setNames(vl, labels)
      
      png(filename=paste(plotdir, "/", sprintf("%05d", p), "_box.png", sep=""), 320, 240)
      boxplot(vl, col=c("green",rep("red",length(vl)-1)))
      dev.off()
      ...
      png(filename=paste(plotdir, "/", sprintf("%05d", p), ".png", sep=""), 320, 240)
      ...
      for (i in 1:length(patients)){
        if (i%%2==0) {
          text(-10, length(patients)+1.2-i, paste("P", patients[i], sep=""), cex=1.3)
        } else {
          text(10, length(patients)+1.2-i, paste("P", patients[i], sep=""), cex=1.3)
        }
        #ints.p=ints.case[grep(paste(case_label, i, "_", sep=""), names(ints.case))]
        ints.p=ints.case[grep(paste(case_label, patients[i], ".", sep=""), names(ints.case),fixed=TRUE)]
